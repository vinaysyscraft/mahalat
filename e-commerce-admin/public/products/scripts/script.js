...
function displayProducts(products) {
  ...
  <td class="py-4 px-4 text-gray-900 dark:text-gray-300">${product.name || 'N/A'}</td>
  <td class="py-4 px-4 description text-gray-900 dark:text-gray-300">${product.description || 'N/A'}</td>
  
  <td class="py-4 px-4 text-gray-900 dark:text-gray-300">
    ${product.category?.name || 'No Category'},<br>
    ${product.subcategory?.name || 'No Subcategory'},<br>
  ...
  <td class="py-4 px-4 text-gray-900 dark:text-gray-300">${product.styleNo || 'N/A'}</td>
  <td class="py-4 px-4 text-gray-900 dark:text-gray-300">${product.productSku || 'N/A'}</td>
  
  <td class="py-4 px-4 text-gray-900 dark:text-gray-300">${Array.isArray(product.colors) ? product.colors.join(', ') : 'N/A'}</td>
  <td class="py-4 px-4 text-gray-900 dark:text-gray-300">${Array.isArray(product.sizes) ? product.sizes.join(', ') : 'N/A'}</td>
...
async function editProduct(id) {
  ...
  try {
    ...
    const subsubcategorySelect = document.getElementById('edit-subsubcategory');
    ...
    // Handle subcategory change to load sub-subcategory
    subcategorySelect.addEventListener('change', (e) => {
      const selectedSubId = e.target.value;
      const selectedCategory = allCategories.find(cat => cat._id === categorySelect.value);
      const selectedSubcategory = selectedCategory?.subcategories.find(sub => sub._id === selectedSubId);
    ...
    function loadSubsubcategories(subsubcategories) {
      subsubcategorySelect.innerHTML = '';
      subsubcategories.forEach(sub => {
        const option = document.createElement('option');
        option.value = sub._id;
        option.textContent = sub.name;
    ...
    if (selectedCategory) {
      loadSubcategories(selectedCategory.subcategories);
      const selectedSubcategory = selectedCategory.subcategories.find(sub => sub._id === product.subcategory._id);
    ...
    document.getElementById('edit-styleNo').value = product.styleNo || '';
    ...
    form.onsubmit = async (e) => {
      ...
      try {
        ...
        formData.append('styleNo', document.getElementById('edit-styleNo').value || ''); // ✅ Add this line

        formData.append('colors', JSON.stringify(selectedColors));
        formData.append('sizes', JSON.stringify(selectedSizes));

        // Main image
        const mainImageFile = document.getElementById('edit-image').files[0];
        if (mainImageFile) {
          formData.append('image', mainImageFile);
        }

        // Gallery images
        const galleryFiles = document.getElementById('edit-gallery')?.files;
